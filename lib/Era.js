function Era(hpd,dpw) {
  ...
  this.config = {
    ...
    //Hours Per Day preset.
    hoursPerDay: (!hpd) ? 8 : hpd
  }

  if(dpw!==undefined) {
    this.config.hoursPerDay = (this.config.hoursPerDay/5) * dpw;
  }
  ...
  this.now = function() {
    ...
    if(day>=6){
      ...
      dur += (7-(day-1))*this.config.dayInMiliseconds; 
      n = new Date(dur);
    }
  ...
  this.evaluate = function(_task) {
    ...
    if(firstInterferor!==null){
      ...
      if(tempTaskEnd>firstInterferor.start) {
        ...
        //Create new task part based on partial origin.
  ...
  this.end = function() {
    that.timeline = [];
    that._timeline = [].concat(that.tasks);
